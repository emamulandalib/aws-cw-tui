    ...
    ui_span!("main_render", {
        ...
        match app.state {
            AppState::ServiceList => {
                ui_span!("service_list", render_service_list(f, app));
            }
            AppState::InstanceList => {
                ui_span!("instance_list", render_rds_list(f, app));
            }
            AppState::MetricsSummary => {
                ui_span!("metrics_summary", render_metrics_summary(f, app));
            }
            AppState::InstanceDetails => {
                ui_span!("instance_details", render_instance_details(f, app));
            }
        }
    });
}
