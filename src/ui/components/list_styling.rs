use crate::ui::themes::{UnifiedTheme, ComponentTheme};
use ratatui::{
    ...
    fn apply_layout_styling(&self, lines: Vec<Line<'static>>) -> Vec<Line<'static>> {
        match self.layout_style {
            ...
            LayoutStyle::Card => {
                ...
                for line in lines {
                    let mut bordered_spans = vec![
                        Span::styled(" | ".to_string(), Style::default().fg(self.colors.separator)),
                    ];
                    bordered_spans.extend(line.spans);
                    bordered_spans.push(Span::styled(" |".to_string(), Style::default().fg(self.colors.separator)));
...
pub mod utilities {
    ...
    pub fn create_status_message(message: &str, message_type: StatusIndicator, _colors: &ListColors) -> String {
