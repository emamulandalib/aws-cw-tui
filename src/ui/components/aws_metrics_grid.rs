...
impl UnifiedMetricData {
    ...
    pub fn get_display_name(&self) -> String {
        match self {
            UnifiedMetricData::Legacy(data) => {
                MetricRegistry::get_definition(&data.metric_type).name.to_string()
            }
    ...
    fn map_dynamic_to_metric_type(metric_name: &str) -> Option<MetricType> {
        match metric_name {
            ...
            "ApproximateNumberOfMessagesVisible" => Some(MetricType::ApproximateNumberOfMessagesVisible),
            "ApproximateNumberOfMessagesNotVisible" => Some(MetricType::ApproximateNumberOfMessagesNotVisible),
...
impl AwsMetricsGrid {
    ...
    pub fn render(f: &mut Frame, app: &App, area: Rect, theme: &UnifiedTheme) {
        ...
        info!("METRICS_GRID: Found {} unified metrics", unified_metrics.len());
        ...
        let visible_metrics: Vec<UnifiedMetricData> = unified_metrics[start_index..end_index].to_vec();
