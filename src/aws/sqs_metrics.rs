...
pub async fn fetch_sqs_metrics(queue: &SqsQueue, time_range: &TimeRange) -> Result<SqsMetricData> {
            ...
            Err(_e) => {
                ...
                match metric_name {
                    ...
                    | "NumberOfMessagesInDlq" => {
                        ...
                        if let Ok((visible_messages, not_visible_messages, _delayed_messages)) =
                            get_current_queue_metrics(queue).await
                        {
                            let current_value = match metric_name {
                                "ApproximateNumberOfMessages" => {
                                    visible_messages + not_visible_messages
                                }
                            ...
                            match metric_name {
                                ...
                                "ApproximateNumberOfMessagesVisible" => {
                                    metric_data.approximate_number_of_messages_visible =
                                        current_value;
                                    metric_data.messages_visible_history = synthetic_history;
                                }
                                "ApproximateNumberOfMessagesNotVisible" => {
                                    metric_data.approximate_number_of_messages_not_visible =
                                        current_value;
        ...
        get_current_queue_metrics(queue).await
    {
        ...
        if metric_data.messages_not_visible_history.is_empty() {
            ...
            metric_data.messages_not_visible_history =
                create_synthetic_history(not_visible_messages, 12);
        }
        if metric_data.queue_depth_history.is_empty() {
            ...
            metric_data.queue_depth_history =
                create_synthetic_history(visible_messages + not_visible_messages, 12);
        }
            ...
            Ok((values, times)) => {
                ...
                match metric_name {
                    "ApproximateNumberOfGroupsWithInflightMessages" => {
                        metric_data.approximate_number_of_groups_with_inflight_messages =
                            current_value;
    ...
    if let Some(results) = response.metric_data_results {
        if let Some(result) = results.first() {
            if let (Some(vals), Some(times)) = (&result.values, &result.timestamps) {
                ...
                let mut paired_data: Vec<(f64, SystemTime)> = vals
                    .iter()
                    .zip(times.iter())
                    .map(|(&value, timestamp)| {
                        (
            ...
            if let Some(datapoints) = recent_stats.datapoints {
                for datapoint in datapoints {
                    if let (Some(value), Some(timestamp)) =
                        (get_stat_value(&datapoint, &statistic), &datapoint.timestamp)
                    {
                        values.push(value);
...
fn calculate_time_range(time_range: &TimeRange) -> (SystemTime, SystemTime) {
    ...
    let duration = match time_range.unit {
        crate::aws::time_range::TimeUnit::Minutes => {
            Duration::from_secs(time_range.value as u64 * 60)
        }
        crate::aws::time_range::TimeUnit::Hours => {
            Duration::from_secs(time_range.value as u64 * 3600)
        }
        crate::aws::time_range::TimeUnit::Days => {
            Duration::from_secs(time_range.value as u64 * 24 * 3600)
        }
        crate::aws::time_range::TimeUnit::Weeks => {
            Duration::from_secs(time_range.value as u64 * 7 * 24 * 3600)
        }
        crate::aws::time_range::TimeUnit::Months => {
            Duration::from_secs(time_range.value as u64 * 30 * 24 * 3600)
        }
    };
...
fn get_dlq_name(queue: &SqsQueue) -> Option<String> {
    ...
    queue.attributes.get("RedrivePolicy").and_then(|policy| {
...
async fn get_current_queue_metrics(queue: &SqsQueue) -> Result<(f64, f64, f64)> {
    ...
    let response = sqs_client
        ...
        .await
        .map_err(|e| {
            AwsErrorHandler::handle_aws_error(e, "get SQS queue attributes", "SQS read permissions")
        })?;
