...
impl MetricServiceFactory {
    ...
    pub fn register_provider(&mut self, service_type: AwsService, provider: Box<dyn MetricProvider>) {
    ...
    pub fn get_provider(&self, service_type: &AwsService) -> Result<&dyn MetricProvider> {
        self.providers
            ...
            .ok_or_else(|| anyhow!("No provider registered for service type: {:?}", service_type))
    }
