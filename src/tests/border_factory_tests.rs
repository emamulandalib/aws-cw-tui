...
mod tests {
    ...
    use crate::ui::themes::{UnifiedTheme, warm_sunset_colors};
    ...
    fn test_validate_border_consistency() {
        ...
        assert!(border_factory::validate_border_consistency(&focused_style, &theme, true));
        
        // Test valid unfocused style
        let unfocused_style = Style::default().fg(theme.border);
        assert!(border_factory::validate_border_consistency(&unfocused_style, &theme, false));
        
        // Test invalid style (wrong color)
        let invalid_style = Style::default().fg(Color::Red);
        assert!(!border_factory::validate_border_consistency(&invalid_style, &theme, true));
        assert!(!border_factory::validate_border_consistency(&invalid_style, &theme, false));
        
        // Test style with no color
        let no_color_style = Style::default();
        assert!(!border_factory::validate_border_consistency(&no_color_style, &theme, true));
        assert!(!border_factory::validate_border_consistency(&no_color_style, &theme, false));
    }
    ...
    fn test_page_specific_border_behavior() {
        ...
        let non_metrics_page_always_unfocused = border_factory::create_theme_border_style(&theme, false);
        let non_metrics_page_ignore_focus = border_factory::create_theme_border_style(&theme, false);
        
        // Both should be the same (unfocused color) regardless of focus parameter for non-metrics pages
        assert_eq!(non_metrics_page_always_unfocused.fg, Some(Color::Rgb(234, 226, 183))); // #EAE2B7
        assert_eq!(non_metrics_page_ignore_focus.fg, Some(Color::Rgb(234, 226, 183))); // #EAE2B7
    }
